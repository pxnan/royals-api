...
from preprocessing import preprocess   # import di luar agar tidak diulang
...
# ==================== Global variables untuk model dan cache ====================
model_qa = None
vectorizer_qa = None
answers = []
pertanyaan_list = []
kategori_list = []
X_all_questions_tfidf = None        # Precomputed TF-IDF matrix untuk semua pertanyaan
_models_loaded = False

# ==================== Connection Pool MySQL (opsional, mengurangi latency) ===================
mysql_pool = None
try:
    ...
    logger.warning(f"Could not create MySQL pool: {e}")

def get_db_connection():
    """Ambil koneksi dari pool, atau buat koneksi biasa jika pool gagal."""
    if mysql_pool:
        try:
...
# ===================== Fungsi untuk menyimpan unknown question ====================
...
# ===================== LOAD & SAVE MODEL (dengan kompresi gzip) ====================
def save_model_to_supabase(model_data):
    """Simpan model terkompresi ke Supabase Storage."""
    if supabase is None:
    ...
    try:
        ...
        try:
            supabase.storage.from_(BUCKET_NAME).upload(MODEL_FILE, compressed,
                                file_options={"content-type": "application/octet-stream"})
        except Exception:
            supabase.storage.from_(BUCKET_NAME).update(MODEL_FILE, compressed,
                                file_options={"content-type": "application/octet-stream"})
        logger.info(f"Model saved to Supabase (compressed size: {len(compressed)/1024:.1f} KB)")
        return True
    except Exception as e:
...
def load_model_from_supabase():
    """Load model terkompresi dari Supabase."""
    if supabase is None:
        logger.warning("Supabase not configured")
        return None
    try:
        compressed = supabase.storage.from_(BUCKET_NAME).download(MODEL_FILE)
        model_bytes = gzip.decompress(compressed)
        model_data = pickle.loads(model_bytes)
        logger.info(f"Model loaded from Supabase (compressed size: {len(compressed)/1024:.1f} KB)")
        return model_data
    except Exception as e:
...
def load_dataset_from_db():
    """Load dataset dari database (fallback jika model tidak ada)."""
...
def precompute_tfidf_matrix():
    """Hitung dan simpan TF-IDF matrix untuk semua pertanyaan (cache)."""
    global X_all_questions_tfidf, vectorizer_qa, pertanyaan_list
    if vectorizer_qa is None or not pertanyaan_list:
        X_all_questions_tfidf = None
        return
    try:
        processed_all = [preprocess(q) for q in pertanyaan_list]
        X_all_questions_tfidf = vectorizer_qa.transform(processed_all)
        logger.info("Precomputed TF-IDF matrix for all questions")
    except Exception as e:
        logger.error(f"Error precomputing TF-IDF matrix: {e}")
        X_all_questions_tfidf = None

def load_models_and_data():
    """Load model dan dataset SEKALI di global scope. Panggil saat startup."""
    global model_qa, vectorizer_qa, answers, pertanyaan_list, kategori_list, X_all_questions_tfidf, _models_loaded
    if _models_loaded:
        return

    start_time = time.time()
    # 1. Coba load model dari Supabase
    ...
    if model_data:
        ...
        # Precompute TF-IDF matrix untuk semua pertanyaan
        precompute_tfidf_matrix()
        _models_loaded = True
        logger.info(f"Model loaded from Supabase: {len(pertanyaan_list)} questions, took {time.time()-start_time:.2f}s")
        return

    # 2. Fallback: load dataset dari database (tanpa model)
    logger.warning("No model available, loading dataset from database (fallback)")
    load_dataset_from_db()
    _models_loaded = True
    logger.info(f"Dataset loaded from DB: {len(pertanyaan_list)} questions, took {time.time()-start_time:.2f}s")

# ==================== LOAD MODEL SAAT STARTUP (SEKALI) ====================
logger.info("Starting application, loading models and data...")
...
def warmup():
    """Endpoint untuk menjaga fungsi tetap hangat (panggil setiap 5 menit)."""
    return jsonify({'status': 'warm', 'model_loaded': model_qa is not None, 'questions': len(pertanyaan_list)})
...
def chat():
    ...
    # 5. Gunakan precomputed TF-IDF matrix (jika ada) untuk skor kedekatan
    global X_all_questions_tfidf
    if X_all_questions_tfidf is None:
        # fallback: hitung ulang
        processed_all = [preprocess(q) for q in pertanyaan_list]
        X_all_questions_tfidf = vectorizer_qa.transform(processed_all)
        logger.info("Recomputed TF-IDF matrix on the fly (should not happen often)")
...
# ==================== ENDPOINT CHAT N8N (juga dioptimasi) ====================
...
def chat_n8n_proxy():
    ...
    if contains_profanity(user_input):
        ...
        if X_input_tfidf.nnz > 0:
            ...
            if category_indices:
                ...
                # Ambil 3 alternatif teratas
                top_global_indices = sorted(range(len(predict_answer_score)), key=lambda i: predict_answer_score[i], reverse=True)[:3]
                lines = []
                for rank, idx in enumerate(top_global_indices):
                    if predict_answer_score[idx] > 0.05:
                        lines.append(f"Alternatif {rank+1}:\nPertanyaan: {pertanyaan_list[idx]}\nJawaban: {answers[idx]}")
                if lines:
    ...
    try:
        ...
        logger.error(f"Koneksi n8n gagal: {e}")
        return jsonify({'status': 'success', 'answer': jawaban_dasar_svm}), 200

# ==================== Semua endpoint lain (tidak berubah) ====================
# Karena sangat panjang, saya akan tuliskan ulang endpoint yang penting,
# namun dengan perubahan kecil (menggunakan get_db_connection() yang sudah ada di pool).
...
def handle_ambiguous_unknown():
    ...
    if contains_profanity(original_question):
        return jsonify({'status': 'error', 'jawaban': get_profanity_response(), 'original_question': original_question}), 200
    save_unknown_question(original_question)
    return jsonify({'status': 'unknown', 'jawaban': 'Mohon maaf, saya belum mengerti pertanyaan Anda. Pertanyaan Anda telah saya catat.', 'original_question': original_question}), 200
...
def get_recommendations_by_category():
    ...
    if conn is None:
        return jsonify({'error': 'Database tidak tersedia'}), 500
    cursor = conn.cursor()
    cursor.execute("SELECT pertanyaan FROM dataset WHERE kategori = %s ORDER BY RAND() LIMIT 3", (kategori,))
    rows = cursor.fetchall()
    cursor.close()
    conn.close()
    recommendations = [row[0] for row in rows]
...
# ==================== AUTHENTICATION (tidak diubah, hanya menggunakan get_db_connection) ====================
...
def login():
    ...
    try:
        cursor = get_db_cursor(conn, dictionary=True)
        cursor.execute("SELECT id, username, password, email, full_name, is_active FROM admin WHERE username = %s", (username,))
        ...
        cursor.close()
        conn.close()
        response_admin = {'id': admin['id'], 'username': admin['username'], 'email': admin['email'], 'full_name': admin['full_name']}
        return jsonify({'authenticated': True, 'token': token, 'admin': response_admin, 'expires_in': JWT_EXPIRATION_HOURS * 3600}), 200
    except Exception as e:
...
def verify_token_endpoint():
    ...
    if not payload:
        return jsonify({'authenticated': False}), 401
    return jsonify({'authenticated': True, 'admin_id': payload['admin_id'], 'username': payload['username']}), 200
...
# ==================== KELOLA ADMIN (tidak diubah, hanya menggunakan get_db_connection) ====================
...
def create_admin():
    ...
    try:
        ...
        cursor.execute("INSERT INTO admin (username, password, email, full_name, is_active) VALUES (%s, %s, %s, %s, %s)",
...
def update_admin(admin_id):
    ...
    try:
        ...
        cursor.execute("UPDATE admin SET email=%s, full_name=%s, is_active=%s, updated_at=NOW() WHERE id=%s",
...
def get_all_data():
    ...
    try:
        ...
        data = []
        for row in rows:
            data.append({'index': row['id'] - 1, 'id': row['id'], 'pertanyaan': row['pertanyaan'], 'jawaban': row['jawaban'], 'kategori': row['kategori']})
        ...
        cursor.close()
        conn.close()
        return jsonify({'page': page, 'per_page': per_page, 'total_data': total, 'total_pages': total_pages, 'data': data, 'categories': categories}), 200
    except Exception as e:
...
def get_recent_dataset():
    ...
    try:
        ...
        data = []
        for row in rows:
            data.append({'id': row['id'], 'pertanyaan': row['pertanyaan'], 'jawaban': row['jawaban'][:50] + ('...' if len(row['jawaban']) > 50 else ''), 'jawaban_full': row['jawaban'], 'kategori': row['kategori']})
        return jsonify({'data': data, 'total': len(data)}), 200
    except Exception as e:
...
def tambah_data():
    ...
    try:
        ...
        cursor.close()
        conn.close()
        return jsonify({'message': 'Data berhasil ditambahkan', 'data': {'pertanyaan': pertanyaan, 'jawaban': jawaban, 'kategori': kategori}, 'status': 'success'}), 201
    except Exception as e:
        conn.rollback()
...
def update_data():
    ...
    try:
        if item_id is not None:
            cursor = conn.cursor()
            cursor.execute("UPDATE dataset SET pertanyaan=%s, jawaban=%s, kategori=%s WHERE id=%s", (pertanyaan, jawaban, kategori, item_id))
        ...
        if index is not None:
            ...
            target_id = ids[index]
            cursor = conn.cursor()
            cursor.execute("UPDATE dataset SET pertanyaan=%s, jawaban=%s, kategori=%s WHERE id=%s", (pertanyaan, jawaban, kategori, target_id))
            conn.commit()
            cursor.close()
            conn.close()
            return jsonify({'message': 'Data berhasil diupdate', 'status': 'success'}), 200
...
# ==================== TRAINING MODEL (dengan kompresi dan update cache) ====================
...
def train_model():
    ...
    try:
        ...
        # Update global variables dan precompute TF-IDF matrix
        global model_qa, vectorizer_qa, answers, pertanyaan_list, kategori_list, X_all_questions_tfidf
        model_qa = model
        ...
        return jsonify({
            'status': 'success',
            'message': 'Model Linear SVM berhasil dilatih dan disimpan (terkompresi)',
...
# ==================== DEBUG DAN ENDPOINT TAMBAHAN (tidak diubah) ====================
...
def cek_csv():
    ...
    return jsonify({'total_lines': len(all_rows), 'total_rows': len(all_rows), 'last_5_rows': all_rows, 'file_path': 'database'}), 200
...
def get_data_by_index(index):
    ...
    row = rows[index]
    return jsonify({'index': index, 'id': row['id'], 'pertanyaan': row['pertanyaan'], 'jawaban': row['jawaban'], 'kategori': row['kategori']}), 200
...
def get_dashboard_stats():
    ...
    try:
        ...
        return jsonify({'total_admin': total_admin, 'total_unknown_questions': total_unknown, 'total_questions': total_questions, 'total_categories': total_categories, 'model_loaded': model_loaded}), 200
    except Exception as e:
...
def search_data():
    ...
    results = []
    for row in rows:
        results.append({'index': row['id'] - 1, 'id': row['id'], 'pertanyaan': row['pertanyaan'], 'jawaban': row['jawaban'], 'kategori': row['kategori']})
    cursor.close()
    conn.close()
...
def get_login_logs():
    ...
    cursor = get_db_cursor(conn, dictionary=True)
    cursor.execute("""SELECT l.*, a.username as admin_username FROM login_logs l LEFT JOIN admin a ON l.admin_id = a.id ORDER BY l.login_time DESC LIMIT %s OFFSET %s""", (per_page, offset))
...
def debug_headers():
    ...
    for key, value in request.headers.items():
        print(f"{key}: {value}")
    return jsonify({"message": "Cek logs untuk detail", "origin": request.headers.get('Origin', 'Tidak ada origin'), "user_agent": request.headers.get('User-Agent', 'Tidak ada user-agent'), "host": request.headers.get('Host', 'Tidak ada host'), "x-forwarded-for": request.headers.get('X-Forwarded-For', 'Tidak ada info IP')}), 200
